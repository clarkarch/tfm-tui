...
- UI lives in `src/index.ts` (imperative, module-level build; runtime mutation via ids + `findDescendantById`) plus `src/style.ts`, the surface-styling seam: every panel/tile/row/button bg+border decision goes through its builders (`chromeSurface`/`tileSurface`/`rowSurface`/`btnSurface`/`slotBg` + `applySurface` for post-mount flips). Never hand-roll bg ternaries at call sites — both `ui-style` modes must stay consistent.
- `ui-style` (ui): `"solid"` (default) fills panels; `"outline"` draws rounded borders on chrome panels with NO rest-state fills anywhere (hover/selected/cut keep their fills — tiles are 3..10 rows and a border ring needs 2, yoga reserves it via setBorder, so rings would clip tile content). Outline sidebar insets children by 2 cells → inner fixed widths go through `sideInnerW()`, and rethemeChrome re-applies surfaces + widths when the mode flips at runtime. Input fields keep fills in both modes (InputRenderable has no border support).

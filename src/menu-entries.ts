...
import { setBookmarked, loadSystemPlaces, type Place } from "./places";
...
export const makeMenuEntries = (ctx: MenuEntriesCtx) => {
  ...
  const sidebarEntriesFor = (place: Place, _x: number, _y: number): ListEntry[] => {
    ...
    if (target) {
      entries.push({ icon: "folder", label: "Open", action: () => { ctx.closeFileMenu(); ctx.navigate(target); } });
      entries.push({ icon: "terminal", label: "Open Terminal Here", action: () => { ctx.closeFileMenu(); ctx.openTerminalHere(target); } });
      // paste into real places (not virtual views, not the trash)
      if (!place.scheme && target !== trashFiles()) {
        entries.push({ icon: "content-paste", label: pasteLabel(ctx.clipboard()?.items.length ?? 0), action: () => { ctx.closeFileMenu(); ctx.pasteSmart(target); } });
      }
      if (target === trashFiles()) {
        ...
        entries.push({ icon: "bookmark", label: "Remove bookmark", action: () => {
          ctx.closeFileMenu();
          void setBookmarked(target, false).then(() => loadSystemPlaces()).then(() => ctx.renderAll());
        } });
      }
    }
    if (place.ejectable && place.device) {
      entries.push({ icon: "eject", label: "Eject", action: () => { ctx.closeFileMenu(); ctx.ejectDevice(place.device!); } });
    }
    if (!target && place.mountDevice) {
      entries.push({ icon: "usb", label: "Mount", action: () => { ctx.closeFileMenu(); ctx.mountDevice(place.mountDevice!); } });
    }
    return entries;
  };
  ...
  const fileEntriesFor = (targetPath: string, isDir: boolean, _x: number, _y: number): ListEntry[] => {
    ...
    if (ctx.inTrashView()) {
      ...
      entries.push(
        { icon: "folder", label: `Restore${inSel && targets.length > 1 ? ` ${targets.length} items` : ""}`, action: () => { ctx.closeFileMenu(); ctx.restoreFromTrash(targets.map((t) => t.path)); } },
        { icon: "eye", label: "Open", action: () => { ctx.closeFileMenu(); ctx.openFileDefault(targetPath); } },
        { icon: "trash-can", label: `Delete permanently`, action: () => { ctx.closeFileMenu(); ctx.confirmDeleteForever(targets.map((t) => t.path)); } },
      );
      return entries;
    }
    if (isDir) entries.push({ icon: "folder", label: "Open", action: () => { ctx.closeFileMenu(); ctx.navigate(targetPath); } });
    else entries.push({ icon: "eye", label: "Open", action: () => { ctx.closeFileMenu(); ctx.openFileDefault(targetPath); } });
    ...
    entries.push(
      { icon: "content-copy", label: `Copy${nSuffix}`, action: () => { ctx.closeFileMenu(); ctx.setClipboard("copy", targets); } },
      { icon: "content-cut", label: `Cut${nSuffix}`, action: () => { ctx.closeFileMenu(); ctx.setClipboard("cut", targets); } },
      ...(isDir
        ? [{ icon: "content-paste", label: pasteLabel(ctx.clipboard()?.items.length ?? 0, " into folder"), action: () => { ctx.closeFileMenu(); ctx.pasteSmart(targetPath); } } satisfies ListEntry]
        : []),
  ...
  const sortEntries = (): ListEntry[] => {
    ...
    const pick = (key: SortMode, naturalAsc: boolean): void => {
      ...
      else { ctx.sortState.sortBy = key; ctx.sortState.sortAsc = naturalAsc; }
      void ctx.renderGrid();
    };
  ...
  const emptyAreaEntries = (_x: number, _y: number): ListEntry[] => {
    ...
    entries.push(
      { icon: "file", label: "New File", action: () => { ctx.closeFileMenu(); ctx.startInlineCreate("file"); } },
      { icon: "folder-plus", label: "New Folder", action: () => { ctx.closeFileMenu(); ctx.startInlineCreate("folder"); } },
      selectAllEntry(),
      { icon: "content-paste", label: pasteLabel(ctx.clipboard()?.items.length ?? 0), action: () => { ctx.closeFileMenu(); ctx.pasteSmart(ctx.cwd()); } },
      { icon: "information", label: "Properties…", action: () => { ctx.closeFileMenu(); ctx.openProperties(ctx.cwd()); } },
      // nautilus puts shell access in its own group at the bottom
      { sep: true, label: "", action: () => {} },
      { icon: "terminal", label: "Open Terminal Here", action: () => { ctx.closeFileMenu(); ctx.openTerminalHere(); } },
    );
    return entries;
  };

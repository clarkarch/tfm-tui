...
export type GridMenuEntry = { icon?: string; label: string; hint?: string; hintIcon?: string; action: () => void; sep?: boolean };
  ...
  return (e: { isDir: boolean }, key: string, idx: number) => {
    ...
    return {
      onMouseDown: (ev: any) => {
        ...
        if (ev.button === 2) {
          ...
          if (!ctx.tileRefs.get(key)?.selected) {
            ...
            if (r) { r.selected = true; ctx.setTileVisual(key, 2); }
            ctx.updateSelectionStatusReal();
            void ctx.renderPreview();
          }
        ...
        if (refs) {
          if (wasSelected && prevSel.length > 1) {
            for (const s of prevSel) {
              ...
              if (r2) { r2.selected = true; ctx.setTileVisual(s.path, 2); }
            }
          } else {
            refs.selected = true;
        ...
        ctx.log(`tile mousedown ${key} wasSel=${wasSelected} prevN=${prevSel.length} -> keys=${gridDrag.keys.length} ctrl=${gridDrag.ctrl}`);
      },
      onMouseUp: () => { if (gridDrag.keys) { ctx.log("tile mouseup -> cleanup scheduled"); commitPendingCtrlToggle(ctx); scheduleDragCleanup(ctx); } },
      ...
      onMouseDrag: (ev: any) => {
        ...
        if (!gridDrag.active && (Math.abs(ev.x - gridDrag.startX) > 1 || Math.abs(ev.y - gridDrag.startY) > 1)) {
          ...
          for (const k of gridDrag.keys) {
            ...
            if (r && !r.selected) { r.selected = true; ctx.setTileVisual(k.path, 2); }
          }
      ...
      onMouseDrop: () => {
        ...
        ctx.log(`tile drop keys=${keys?.length ?? -1}[${keys?.map((k) => k.path.split("/").pop()).join(",") ?? ""}] dest=${dest} isDir=${e.isDir}`);
        finishDragState(ctx);
        if (keys && dest && e.isDir) void ctx.moveInto(dest, keys.filter((k) => k.path !== dest));
      },
      onMouseOver: () => {
        if (gridDrag.active) {
          const draggingSelf = !!gridDrag.keys?.some((k) => k.path === key);
          if (e.isDir && !draggingSelf) { ctx.log(`hover target set ${key}`); gridDrag.dropTarget = key; ctx.setTileVisual(key, 2); }
          return;
        }
        const refs = ctx.tileRefs.get(key);
